...
import { SITE_NAME, getAbsoluteUrl, resolveMediaReference, type EntryType, type Locale } from '@/lib/site';
...
export function buildEntryMetadata(entry: ContentEntry, locale: Locale = entry.locale): Metadata {
  ...
  const ogImage = entry.ogImage ? resolveMediaReference(entry.ogImage) : undefined;
