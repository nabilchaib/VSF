...
import { SITE_NAME, getAbsoluteUrl, getMediaUrl, type EntryType, type Locale } from '@/lib/site';
...
export function buildEntryMetadata(entry: ContentEntry, locale: Locale = entry.locale): Metadata {
  ...
  const ogImage = entry.ogImage?.startsWith('http') ? entry.ogImage : entry.ogImage ? getMediaUrl(entry.ogImage) : undefined;
