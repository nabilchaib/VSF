...
export function PageHero({ title, eyebrow, subtitle, image, align = 'center', children }: HeroConfig) {
  return (
    <section className="relative isolate overflow-hidden">
      ...
      />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(117,190,43,0.24),transparent_28%)]" />
      <div className="relative mx-auto flex min-h-[340px] w-full max-w-6xl flex-col justify-center px-6 py-20 lg:min-h-[460px] lg:px-10">
        <div className={`max-w-4xl space-y-5 ${align === 'center' ? 'mx-auto text-center' : 'text-left'}`}>
          {eyebrow ? (
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-sand/80">{eyebrow}</p>
          ) : null}
          <h1 className="text-4xl font-bold uppercase tracking-[-0.04em] text-sand md:text-6xl lg:text-7xl">{title}</h1>
          {subtitle ? <p className="mx-auto max-w-3xl text-lg leading-8 text-sand/82">{subtitle}</p> : null}
          {children}
        </div>
      </div>
    </section>
